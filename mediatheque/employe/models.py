...
class Member(models.Model):
    ...
    def nombre_emprunts_actifs(self):
        return Emprunt.objects.filter(membre=self, retourne=False).count()

    def a_emprunts_en_retard(self):
        emprunts = Emprunt.objects.filter(membre=self, retourne=False)
        for emprunt in emprunts:
            if emprunt.date_retour_prevu < timezone.now().date():
                return True
        return False
...
class Media(models.Model):
    ...
    type = models.CharField(max_length=50, choices=TYPE_CHOICES, default='Livre')
    disponible = models.BooleanField(default='Oui')
